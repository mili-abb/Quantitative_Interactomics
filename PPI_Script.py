...
def plot_complexome_analysis(df, species, bait_name, pKapp_col, log_complexome_col, save_fig=True):
    ...
    y_labels = ['pKapp (µM)', 'Concentration (µM)', '[Complex] (µM)']
    ...
    #fig.suptitle(f"{species} {bait_name} Complexome Analysis", fontsize=14, fontweight='bold')
    fig.suptitle(f"{species} {bait_name} Complexome Analysis", fontsize=11, fontweight='bold')
    ...
    for i, sort_by in enumerate(sort_criteria):
        ...
        for j, (data_col, color, ylabel, title) in enumerate(zip(
            ...
            axs[i, j].bar(x_indices, sorted_df[data_col], color=color)
            #axs[i, j].set_ylabel(ylabel, fontsize=10)
            axs[i, j].set_ylabel(ylabel, fontsize=8)
            axs[i, j].set_xlabel('Binding Partners', fontsize=8)

            #axs[0, j].set_title(title, fontsize=12, fontweight='bold')
            axs[0, j].set_title(title, fontsize=9, fontweight='bold')
    ...
    if save_fig:
        output_dir = "/home/gogllab/Desktop/Projet_SRC_HCK/Python_First_Part/output"
...
def analyze_and_plot_correlation(csv_file, output_file_prefix, species_x, col_x, species_y, col_y, title, graph_type, annotate_genes=False):
    """
    ...
    lower_pred_band, upper_pred_band = calculate_prediction_bands(px, x, y, popt, linear_function, conf=0.95)

    
    # Create the plot
    fig, ax = plt.subplots(1, 1, figsize=(4, 4))
    ax.plot(px, nom, c='lightgray', label='y = a x + b')
    ax.plot(px, nom - 1.96 * std, color='dimgray', lw=1)
    ax.plot(px, nom + 1.96 * std, color='dimgray', lw=1)
    
    # Set point color based on graph type
    if graph_type in ['affinity', 'delta_pKapp']:
        point_color = '#016c59'
    elif graph_type in ['log_complexome', 'delta_complexome']:
        point_color = '#67a9cf'
    else:
        point_color = '#4F8EC5'  # default color

    ax.plot(x, y, 'o', color=point_color, markersize=4, markeredgecolor='black', markeredgewidth=0.5)
   ...
   # Annotate points with gene IDs if requested
    if annotate_genes:
        ...
        if gene_col in data_clean.columns:
            for i, row in data_clean.iterrows():
                ax.annotate(row[gene_col], (row[x_col], row[y_col]), fontsize=8, alpha=0.7)
        else:
            print("Column 'Gene_ID' doesn't exist for the specified species.")
    
    # Configure axes based on graph type
    if graph_type == 'affinity':
        ax.set_xlabel(r'p$\it{K}_{\rm app}$ ', fontsize=14)
        ax.set_ylabel(r'p$\it{K}_{\rm app}$ ', fontsize=14)
        ax.axis([3.5, 6.5, 3.5, 6.5])
    
    elif graph_type == 'delta_pKapp':
        ax.set_xlabel(r'$\Delta pK_{\rm app}$ (Human)', fontsize=14)
        ax.set_ylabel(r'$\Delta pK_{\rm app}$ (Mouse)', fontsize=14)
        ax.axhline(0, color='gray', linestyle='--', linewidth=0.8)
        ax.axvline(0, color='gray', linestyle='--', linewidth=0.8)
        ax.set_xlim(-1.2, 0.5)
        ax.set_ylim(-1, 0.5)

    elif graph_type == 'log_complexome':
        ax.set_xlabel('log[Complexe] (μM)', fontsize=12)
        ax.set_ylabel('log[Complexe] (μM)', fontsize=12)
        ax.set_xlim(-6, 0)
        ax.set_ylim(-6, 0)

    elif graph_type == 'log_ratio_complexome':
        ax.set_xlabel('Ratio Mouse_log_comp', fontsize=14)
        ax.set_ylabel('Ratio Human_log_comp', fontsize=14)
        ax.set_xlim(-2, 1)
        ax.set_ylim(-2, 0)
    ...
    # Save the plot
    output_path = f'output/{output_file_prefix}.png'
...
def plot_venn_partners(df, output_file_prefix, bait=None, species=None, interspecies=False):
    """
    Create a Venn diagram showing overlap between binding partners with Jaccard similarity.
    
    - If interspecies=False: Compare SRC vs HCK within one species (Human or Mouse)
    - If interspecies=True: Compare Human vs Mouse for one bait protein (SRC or HCK)
    
    Parameters:
        df (DataFrame): DataFrame with multi-level columns (species, column_name)
        output_file_prefix (str): Prefix for the output image file
        bait (str, optional): 'SRC' or 'HCK' (required if interspecies=True)
        species (str, optional): 'Human' or 'Mouse' (required if interspecies=False)
        interspecies (bool): Whether to compare between species (True) or between baits (False)
    ...
    if interspecies:
        if bait not in ['SRC', 'HCK']:
            print("Please specify 'bait' = 'SRC' or 'HCK' for inter-species comparison.")
            return None

        # Get sets of binding partners for the specified bait in each species
        ...
        # Create Venn diagram for inter-species comparison
        v= venn2([partners_human, partners_mouse], set_labels=('Human', 'Mouse'))

        # Calculate Jaccard similarity
        intersection_size = len(partners_human.intersection(partners_mouse))
        union_size = len(partners_human.union(partners_mouse))
        jaccard = intersection_size / union_size if union_size > 0 else 0
        
        # Add Jaccard similarity to the plot title
        plt.title(f"{bait} Partners: Human vs Mouse\nJaccard Similarity: {jaccard:.3f}")
        
        set_a = partners_human
        set_b = partners_mouse
        labels = ('Human', 'Mouse')

    else:
        if species not in ['Human', 'Mouse']:
            print("Please specify 'species' = 'Human' or 'Mouse' for intra-species comparison.")
            return None

        # Get sets of binding partners for each bait in the specified species
        ...
        # Create Venn diagram for intra-species comparison
        v= venn2([partners_src, partners_hck], set_labels=('SRC', 'HCK'))
        
        # Calculate Jaccard similarity
        intersection_size = len(partners_src.intersection(partners_hck))
        union_size = len(partners_src.union(partners_hck))
        jaccard = intersection_size / union_size if union_size > 0 else 0
        
        # Add Jaccard similarity to the plot title
        plt.title(f"{species} SRC vs HCK Partners\nJaccard Similarity: {jaccard:.3f}")
        
        set_a = partners_src
        set_b = partners_hck
        labels = ('SRC', 'HCK')
    
    # Add counts to the Venn diagram labels
    if v:
        # Add set sizes to labels
        plt.annotate(f'{len(set_a)}', xy=v.get_label_by_id('10').get_position(), 
                     xytext=(-10,0), ha='center', textcoords='offset points')
        plt.annotate(f'{len(set_b)}', xy=v.get_label_by_id('01').get_position(), 
                     xytext=(10,0), ha='center', textcoords='offset points')
        
        # Add intersection size if it exists
        if intersection_size > 0 and v.get_label_by_id('11') is not None:
            plt.annotate(f'{intersection_size}', xy=v.get_label_by_id('11').get_position(), 
                         ha='center')
    ...
    print(f"Set A ({labels[0]}): {len(set_a)} elements")
    print(f"Set B ({labels[1]}): {len(set_b)} elements")
    print(f"Intersection: {intersection_size} elements")
    print(f"Union: {union_size} elements")
